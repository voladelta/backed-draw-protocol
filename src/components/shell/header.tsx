...
export function Header({
  ...
  return (
    <header className={cn("site-header sticky top-0 z-40", className)}>
      <div className="site-header-inner mx-auto flex h-[72px] max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        <a aria-label="Draw protocol home" className="flex shrink-0 items-center gap-2.5" href="/">
          {brand ?? <BrandMark />}
        </a>

        <nav aria-label="Primary navigation" className="site-nav hidden items-center gap-1 md:flex">
          {navigation.map((item) => (
            <a
              className={cn(
        ...
        <details className="mobile-nav group relative md:hidden">
          <summary className="mobile-nav-trigger flex size-10 cursor-pointer list-none items-center justify-center rounded-xl border border-white/10 bg-white/[0.05] text-slate-200 transition-[background-color,border-color,color] hover:bg-white/[0.1] [&::-webkit-details-marker]:hidden">
            <Menu aria-hidden className="size-5" />
            <span className="sr-only">Open navigation</span>
          </summary>
          <div className="mobile-nav-menu absolute right-0 top-[calc(100%+12px)] w-64 rounded-2xl border border-white/10 p-2 shadow-2xl backdrop-blur-xl">
